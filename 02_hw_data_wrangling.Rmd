...
incumbency_table <- table(fb_ads$incumbency)
incumbency_table
knitr::kable(incumbency_table)
```
There are 7,014 candidates in the dataset. The table above shows the incumbency status for each candidate.
...
party_incumbent_promote <- fb_ads |>
  ...
  summarize(promote_prop = mean(ad_tone_promote, na.rm = T))
knitr::kable(party_incumbent_promote, digits = 3, col.names = c("Party", "Incumbency", "Proportion of Promoting Ads"))
```
On Average, Democratic Incumbent Candidates sponsored the most promoting ads.
...
plot_q3
```
According to the histogram above, almost none of the US House races have over 10 million ad impressions on Facebook.
...
fb_top_plot <- ggplot(house_top_15,
                      mapping = aes(x = impressions_millions, y = fct_reorder(cand_name_last,impressions_millions))) +
